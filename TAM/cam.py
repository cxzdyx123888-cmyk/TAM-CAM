...
def multimodal_process(raw_img, vision_shape, img_scores, txt_scores, txts, candidates, candi_scores, \
    ...
    if isinstance(vision_shape[0], tuple):
        ...
        for n in range(len(vision_shape)):
            ...
            img_map_ = img_scores[start_idx: end_idx].reshape(t_h, t_w)
        ...
        try:
            ...
            print('Skip text visualization, please check the installation of texlive-xetex.', e)
    ...
    # single img
        ...
        # apply filter
        img_scores = img_scores.reshape(t_h, t_w)
    ...
    # video
        ...
        if vis_width > 0:
            h = int(float(h) / w * vis_width)
            w = int(vis_width)
        mg_scores = np.array([_.reshape(t_h, t_w) for _ in np.array_split(img_scores, b)])
...
def CAM(tokens, vision_shape, logit_list, special_ids, vision_input, \
        processor, save_fn, target_token, img_scores_list, eval_only=False, return_pairs=False):
    ...
    if round_idx == 0 and isinstance(target_token, int):
        for t in range(len(prompt) + 1):
            ...
            img_map = CAM(tokens, vision_shape, logit_list, special_ids, vision_input, processor, \
    ...
    if len(img_scores_list) > 1 and vis_token_idx < len(txt_all):
        ...
        img_scores = img_scores.clip(min=0)
        # L1 distance 当前激活图 vs 前面答案 token 激活图
        d_vals = np.abs(np.stack(img_scores_list_,0) - img_scores.reshape(1,-1)).sum(axis=1)

        # 仅在 return_pairs=True 时返回
        if return_pairs:
            pair_info = (txt_scores_ / txt_scores_.max(), d_vals)
        else:
            pair_info = None
    else:
        pair_info = None
