...
def tam_demo_for_qwen2_vl(image_path, prompt_text, save_dir='vis_results'):
    ...
    for i in range(len(logits)):
        if(i == len(logits) - 1):
            ...
            img_map, pair_info = res
            if pair_info is not None:
                r_vals, d_vals = pair_info
...
if __name__ == "__main__":
    ...
    # 遍历 test_image 文件夹下所有图片
    for img_path in glob.glob("test_image/*.*"):
        if not img_path.lower().endswith((".jpg", ".png", ".jpeg", ".bmp", ".webp")):
            continue
        print(f"start processing {img_path} ...")

        try:
            r_list, d_list = tam_demo_for_qwen2_vl(img_path, "Describe this image. More than 400 words", save_dir='imgs/vis_img_tam')
            if r_list and d_list:
                all_r.extend(r_list)
                all_d.extend(d_list)
    ...
    # 归一化纵轴
    d_norm = d_arr / (d_arr.max() + 1e-8)

    # 过滤只保留 0.05–0.45 范围的点
    mask = (d_norm >= 0.05) & (d_norm <= 0.45)
    r_arr = r_arr[mask]
    d_norm = d_norm[mask]

    # 画散点图
    plt.figure(figsize=(3.5, 6.5))
    plt.scatter(r_arr, d_norm, s=8, alpha=0.5)
    plt.xlabel("Text-to-text correlation")
    plt.ylabel("L1 distance of text-to-vision (CAM) pairs")
    plt.title("Correlated activations")
    plt.grid(alpha=0.2)

    # 设置纵轴范围与刻度
    plt.ylim(0.05, 0.45)
    plt.yticks(np.arange(0.05, 0.5, 0.05))  # 每一格0.05
    ...
    # 拟合趋势线
    if len(r_arr) > 1:
        coef = np.polyfit(r_arr, d_norm, 1)
        xs = np.linspace(0, 1, 100)  # 保证横轴从0到1
        plt.plot(xs, np.polyval(coef, xs), color='black', linewidth=1.2)
    # 自动调整边框大小
    plt.tight_layout() 
    plt.savefig('scatter_vis.png', dpi=300)
    print("Saved figure to scatter_vis.png")
